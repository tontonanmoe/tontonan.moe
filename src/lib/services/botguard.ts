import { fetchFunction } from '$lib/utils/helpers';
...
export class BotguardService {
	...
	private async _initBotguard() {
		try {
			...
			if (!this.bgChallenge) return;

			const interpreterJavascript =
				this.bgChallenge.interpreterJavascript.privateDoNotAccessOrElseSafeScriptWrappedValue;

			if (!interpreterJavascript) {
				console.error(
					'[BotguardService]',
					'Could not get interpreter javascript. Interpreter Hash:',
					this.bgChallenge.interpreterHash
				);
				return;
			}
			...
			console.error('[BotguardService]', 'Failed to initialize BotGuard', error);
			return undefined;
		}
	}
